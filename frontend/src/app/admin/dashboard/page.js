...
export default function AdminDashboard() {
  ...
  async function toggleRegenerate(teamId, currentValue) {
    try {
      await axios.post(`${getApiUrl()}/admin/toggle-regenerate`, { teamId, canRegenerate: !currentValue }, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }});
      fetchSubmissions();
  ...
  return (
    <div className="flex min-h-screen bg-[#f1f5f9] font-sans text-slate-800 uppercase tracking-tight overflow-hidden">
      ...
      <aside className="w-64 bg-[#020617] text-white flex flex-col h-screen sticky top-0 p-5 space-y-8 border-r border-white/5 shadow-2xl">
        <div className="flex items-center gap-3"><div className="w-8 h-8 bg-white/10 rounded-lg flex items-center justify-center font-black text-sm border border-white/10">H</div><div><p className="font-black text-lg tracking-tighter leading-none">HACK@JIT</p><p className="text-[8px] text-slate-500 font-bold tracking-[0.2em] mt-1">ADMIN PORTAL</p></div></div>
        ...
        <nav className="flex-1 space-y-1">
           {[
            ...
            { id: 'configuration', label: 'CONFIG' }
             ...
             <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`w-full text-left px-4 py-3 rounded-xl text-[10px] font-black tracking-widest transition-all flex items-center justify-between group ${activeTab === tab.id ? 'bg-white/10 text-white shadow-lg border border-white/5' : 'text-slate-500 hover:text-white hover:bg-white/5'}`}>
               {tab.label} <div className={`w-1 h-1 rounded-full ${activeTab === tab.id ? 'bg-emerald-500 scale-100' : 'scale-0'}`}></div>
             </button>
           ))}
        </nav>

        <div className="p-4 bg-white/5 rounded-2xl border border-white/5 text-center">
            <p className="text-[8px] text-slate-500 font-black mb-1 opacity-50">CLOCK</p>
            <p className={`text-2xl font-mono font-black ${timer.timerPaused ? 'text-amber-400' : 'text-emerald-400'}`}>{timer.formattedTime}</p>
        </div>
      </aside>
      ...
      <main className="flex-1 p-8 overflow-y-auto space-y-10">
        <header className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
           <div><h1 className="text-2xl font-black text-[#0f172a] tracking-tighter">COMMAND CENTER</h1><p className="text-[9px] font-bold text-slate-400 tracking-[0.3em]">HACKATHON MANAGEMENT v5.2</p></div>
           <div className="flex gap-3">
              <button onClick={handleToggleHalt} className={`px-5 py-2.5 rounded-xl font-black text-[9px] tracking-widest transition-all ${timer.timerPaused ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'}`}>{timer.timerPaused ? 'RESUME EVENT' : 'PAUSE EVENT'}</button>
              <button onClick={handleToggleCertCollection} className={`px-5 py-2.5 rounded-xl font-black text-[9px] tracking-widest transition-all ${stats.config?.allowCertificateDetails ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-400 border border-slate-200'}`}>{stats.config?.allowCertificateDetails ? 'CLOSE CERTS' : 'OPEN CERTS'}</button>
           </div>
        </header>
             ...
             {[
               ...
               { label: 'CERTS READY', val: stats.certificates?.collected || 0, color: 'text-rose-600' }
                ...
                <div key={i} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm relative overflow-hidden group">
                   <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-4">{c.label}</span>
                   <p className={`text-5xl font-black ${c.color} tracking-tighter tabular-nums`}>{c.val}</p>
                </div>
             ))}
          </div>
        )}
        ...
        {activeTab === 'submissions' && (
           <div className="space-y-4 animate-fade-in">
              <div className="flex justify-between items-center">
                 <h2 className="text-xl font-black text-navy tracking-tight">VAULT REGISTRY</h2>
                 <div className="flex bg-white p-1 rounded-xl border border-slate-200 shadow-sm gap-1">
                    {['ALL', 'PENDING', 'SUBMITTED'].map(f => (
                      <button key={f} onClick={() => setSubFilter(f)} className={`px-4 py-1.5 rounded-lg text-[9px] font-black tracking-widest transition-all ${subFilter === f ? 'bg-[#020617] text-white shadow-md' : 'text-slate-400 hover:bg-slate-50'}`}>{f}</button>
                    ))}
                 </div>
              </div>
              <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                 <table className="w-full text-left">
                    <thead className="bg-slate-50 border-b border-slate-100 text-[8px] font-black text-slate-400 uppercase"><tr className="divide-x divide-slate-100"><th className="px-5 py-4">TEAM ENTITY</th><th className="px-5 py-4">STATUS</th><th className="px-5 py-4">LINKS</th><th className="px-5 py-4">CERT DATA</th><th className="px-5 py-4 text-right">CONTROLS</th></tr></thead>
                         ...
                         <tr key={s.id} className="text-[11px] font-bold hover:bg-slate-50 transition-all divide-x divide-slate-50">
                           ...
                           <td className="px-5 py-3">
                              <div className="flex gap-2">
                                {s.pptUrl && <a href={s.pptUrl} target="_blank" className="text-indigo-600 border border-indigo-100 px-2 py-1 rounded bg-indigo-50 hover:bg-indigo-600 hover:text-white transition-all">PPT ↓</a>}
                                {s.prototypeUrl && <a href={s.prototypeUrl} target="_blank" className="text-teal-600 border border-teal-100 px-2 py-1 rounded bg-teal-50 hover:bg-teal-600 hover:text-white transition-all">DEMO ↗</a>}
                              </div>
                           </td>
                           <td className="px-5 py-3">
                              <div className="flex gap-2">
                                 {s.certificates?.map((c, idx) => (
                                    <div key={idx} className="flex items-center gap-1 bg-slate-50 px-2 py-1 rounded border border-slate-100">
                                       <span className="text-[7px] text-slate-400">{c.role[0]}:</span>
                                       {c.certificateUrl ? <a href={c.certificateUrl} target="_blank" className="text-indigo-600 text-[8px] hover:underline">CERT ↓</a> : <span className="text-slate-300 text-[8px]">VOID</span>}
                                    </div>
                                 ))}
                                 {s.certificates?.length > 0 && <button onClick={() => handleGenerateCerts(s.teamId)} className="text-[7px] font-black text-rose-500 uppercase border border-rose-100 px-1.5 rounded hover:bg-rose-500 hover:text-white">RE-GEN ALL</button>}
                              </div>
                           </td>
                           <td className="px-5 py-3 text-right">
                              <div className="flex gap-2 justify-end items-center">
                                 <button onClick={() => toggleRegenerate(s.teamId, s.canRegenerate)} className={`px-3 py-1 rounded text-[8px] font-black tracking-widest border transition-all ${s.canRegenerate ? 'bg-emerald-50 text-emerald-600 border-emerald-100 hover:bg-emerald-600 hover:text-white' : 'bg-rose-50 text-rose-600 border-rose-100 hover:bg-rose-600 hover:text-white'}`}>
                                    {s.canRegenerate ? 'UNLOCK' : 'LOCK'}
                                 </button>
                                 <button onClick={() => handleForceRegenerate(s.teamId)} className="p-1.5 bg-slate-100 rounded text-slate-400 hover:bg-[#020617] hover:text-white transition-all">
                                    <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" /></svg>
                                 </button>
                              </div>
                           </td>
                         </tr>
                 ...
                 {submissions.length === 0 && <div className="p-20 text-center opacity-20"><p className="text-sm font-black uppercase tracking-widest">Repository Empty</p></div>}
              </div>
           </div>
        )}
              ...
              <div className="col-span-4 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm h-fit space-y-6">
                 ...
                 <form onSubmit={handleCreateStatement} className="space-y-3">
                    <div className="grid grid-cols-2 gap-2">
                       <input className="input-sm" placeholder="Q.ID" value={newStatement.questionNo} onChange={e => setNewStatement({...newStatement, questionNo: e.target.value})} required />
                       <input className="input-sm" placeholder="Div" value={newStatement.subDivisions} onChange={e => setNewStatement({...newStatement, subDivisions: e.target.value})} />
                    </div>
                    <input className="input-sm" placeholder="Title" value={newStatement.title} onChange={e => setNewStatement({...newStatement, title: e.target.value})} required />
                    <textarea className="input-sm min-h-[100px]" placeholder="Abstract..." value={newStatement.description} onChange={e => setNewStatement({...newStatement, description: e.target.value})} required />
                    <button className="w-full py-3 bg-[#020617] text-white font-black text-[9px] tracking-widest rounded-xl hover:bg-teal-500 transition-all uppercase shadow-lg">Deploy</button>
                 </form>
              </div>
              <div className="col-span-8 bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden h-fit">
                 <div className="px-6 py-4 border-b border-slate-50 flex justify-between items-center bg-slate-50/50"><h2 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Master Registry</h2><span className="text-[8px] font-bold text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full">{problemStatements.length} MODULES</span></div>
                    ...
                    {problemStatements.map(ps => (
                      <div key={ps.id} className="p-4 hover:bg-slate-50 transition-all flex justify-between items-center group">
                         <div className="flex items-center gap-4 flex-1">
                            ...
                            <div className="truncate"><h4 className="text-[13px] font-black text-navy uppercase truncate tracking-tight">{ps.title}</h4><p className={`text-[8px] font-black uppercase ${ps.allottedTo ? 'text-teal-600' : 'text-amber-500'}`}>{ps.allottedTo ? `Allotted: ${ps.allottedTo}` : 'Unallotted'}</p></div>
                         </div>
                         <button onClick={() => handleDeleteStatement(ps.id)} className="p-2 text-rose-500 opacity-0 group-hover:opacity-100 transition-all hover:bg-rose-50 rounded-lg"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                      </div>
                    ))}
                 </div>
              </div>
           ...
           <div className="grid grid-cols-12 gap-8 animate-fade-in">
              <div className="col-span-4 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm h-fit space-y-6">
                 ...
                 <form onSubmit={handleCreateTeam} className="space-y-3">
                    ...
                    <select className="input-sm font-black text-[9px]" value={newTeam.problemStatementId} onChange={e => setNewTeam({...newTeam, problemStatementId: e.target.value})}>
                        <option value="">-- No Assignment --</option>
                        {problemStatements.map(ps => <option key={ps.id} value={ps.id} disabled={!!ps.allottedTo}>{ps.questionNo}: {ps.title}</option>)}
                    </select>
                    <button className="w-full py-3 bg-[#020617] text-white font-black text-[9px] tracking-widest rounded-xl hover:bg-teal-500 transition-all shadow-lg active:scale-95 uppercase">Instantiate</button>
                 </form>
              </div>
              <div className="col-span-8 bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden h-fit">
           ...
           <div className="grid grid-cols-2 gap-8 animate-fade-in">
              <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-8">
                 ...
                 <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 space-y-4">
                    <div className="flex items-center gap-3"><span className="text-lg">🎓</span><p className="text-[9px] font-black text-indigo-700 tracking-widest uppercase">Certification Lifecycle</p></div>
                    <p className="text-[10px] font-medium text-slate-500 normal-case leading-relaxed italic pr-4">Enable the post-event data acquisition phase for team awards.</p>
                    <button onClick={handleToggleCertCollection} className={`w-full py-4 rounded-xl font-black text-[10px] tracking-widest transition-all shadow-md active:scale-95 uppercase ${stats.config?.allowCertificateDetails ? 'bg-indigo-600 text-white shadow-indigo-100' : 'bg-white text-slate-400 border-2 border-slate-200'}`}>{stats.config?.allowCertificateDetails ? 'Terminate Certification' : 'Initialize Certification'}</button>
                 </div>
              </div>
      ...
      <style jsx global>{`
        ...
        ::-webkit-scrollbar-thumb { background: #e2e8e0; border-radius: 10px; }
      `}</style>
    </div>
  );
}
